...
public class PropertyOperations
	...
	public static boolean validateMultiplicityOfComposite(Property property,
		...
		if (upperBound == LiteralUnlimitedNaturalOperations.UNLIMITED || upperBound > 1) {
...
//			for (Type subsettingContext : property.subsettingContext()) {
			for (Type subsettingContext : subsettingContext(property)) {

//				for (Type spSubsettingContext : subsettedProperty.subsettingContext()) {
				for (Type spSubsettingContext : subsettingContext(subsettedProperty)) {

					if (TypeOperations.conformsTo(subsettingContext,spSubsettingContext)) {
						continue spLoop;
					}
				}
			}
	...
	public static boolean validateSubsettingRules(Property property,
		...
		for (Property subsettedProperty : property.getSubsettedProperty()) {
			...
			int subsettedUpperBound = subsettedProperty.getUpper();

			if (!(type == null
				? subsettedType == null
				: TypeOperations.conformsTo(type,subsettedProperty.getType()))
				|| (subsettedUpperBound != LiteralUnlimitedNaturalOperations.UNLIMITED && (upperBound == LiteralUnlimitedNaturalOperations.UNLIMITED || upperBound > subsettedUpperBound))) {
	...
	public static boolean validateDeploymentTarget(Property property,
		...
		if (!property.getDeployment().isEmpty()) {
	...
	public static boolean isAttribute(Property property, Property p) {
		...
		for (EStructuralFeature.Setting nonNavigableInverseReference : getNonNavigableInverseReferences(p)) {
			...
			if (eObject instanceof Classifier
				&& ((Classifier) eObject).getAttribute().contains(p)) {
...
//					: property.createDefaultValue(null, null,
			: createDefaultValue(property,null, null,
...
//			property.createDefaultValue(null, null,
	...
	static public ValueSpecification createDefaultValue(Property property,String name, Type type,
			EClass eClass) {
			...
			if (association != null) {
				...
				if (memberEnds.size() == 2) {
					...
					if (index != -1) {
						...
						if (!association.getOwnedEnd().contains(otherEnd)
							|| association.getNavigableOwnedEnd().contains(
								otherEnd)) {
	...
	public static boolean isComposite(Property property) {
		return property.getAggregation() == AggregationKindEnum.COMPOSITE;
	}
	...
	public static boolean isNavigable(Property property) {
		...
		return owningAssociation == null
			...
			: owningAssociation.getNavigableOwnedEnd().contains(property);
	}
...
//			property.setStringDefaultValue(newDefault);
			setStringDefaultValue(property,newDefault);
		}
	}
	...
	public static void setOpposite(Property property, Property newOpposite) {
		...
		if (association == null) {
			...
			Package owningPackage = (Package) ElementOperations
				.getOwningElement(property, UMLPackage.Literals.PACKAGE, true);

			if (owningPackage != null) {
				owningPackage.getPackagedElement().add(association);
			}
		}
...
//		if (property.isSetDefault()) {
	...
	public static void setIsNavigable(Property property, boolean isNavigable) {
		...
		if (property.isNavigable() != isNavigable) {
			...
			Collection<Property> navigableOwnedEnds = association
			...
			if (isNavigable) {
				...
				association.getOwnedEnd().add(property);
			}
		}
	}
...
//				&& redefinee.isRedefinitionContextValid(property)) {
			&& RedefinableElementOperations.isRedefinitionContextValid(redefinee,property)) {
			...
			int propUpperBound = prop.getUpper();

			return (propType == null
				? type == null
				: TypeOperations.conformsTo(propType,type))
				&& prop.getLower() >= property.getLower()
				&& (upperBound == LiteralUnlimitedNaturalOperations.UNLIMITED || (propUpperBound != LiteralUnlimitedNaturalOperations.UNLIMITED && propUpperBound <= upperBound))
				&& (property.isDerived()
					? prop.isDerived()
		...
		if (property.eClass().isInstance(p)) {
			...
			return type == null
				? ((Property) p).eGet(UMLPackage.Literals.TYPED_ELEMENT__TYPE,
					false) == null
				: TypeOperations.conformsTo(type,((Property) p).getType());
		}
