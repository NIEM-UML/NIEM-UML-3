...
public class NIEMHelper implements NIEM_ProfileConstants{
	...
	static public boolean isInImportedAsConformantNamespace(Element element){
		Package p=getNearestPackage(element);
		if(p==null)return false;
		return isImportedAsConformantNamespace(p);
	}
	static public boolean isImportedAsConformantNamespace(Package informationModel){
		return getNamespaceIsConformant(informationModel)
				||isXMLPrimitiveTypesLibrary(informationModel)
				||isNamespaceICISM(informationModel)
				||isNamespaceICINTK(informationModel)
				;
	}
	...
	static public void closeSession(boolean isSessionCreated){
		if (!isSessionCreated)
		{
			if(!SessionManager.getInstance().isSessionCreated()){QvtCommonAction.log("ERROR: closeSession - session is no longer active ");return;}else
...
//		if(NIEMHelper.hasStereotype(testClass, NIEMHelper.getCommonStereotype(ChoiceStereotypeName)))return false;
	...
	static public boolean isObjectType(Classifier classifier){
		if(!isComplexType(classifier))return false;
		 if(NIEMHelper.isAssociationType((Classifier)classifier))return false;
		 if(NIEMHelper.isMetadataType((Classifier)classifier))return false;
		 if(NIEMHelper.isAugmentationType((Classifier)classifier))return false;
		 if(NIEMHelper.isPropertyHolder((Classifier)classifier))return false;
		return true;
	}
	static public boolean isAugmentableType(Classifier classifier){
		if(!isComplexType(classifier))return false;
		 //if(NIEMHelper.isAdapterType((Classifier)classifier))return false;
		 if(NIEMHelper.isAssociationType((Classifier)classifier))return true;
		 if(NIEMHelper.isObjectType((Classifier)classifier))return true;
		return true;
	}
	static public boolean isAugmentationPoint(Property property){
		if(!hasAugmentationPointName(property))return false;
		...
		if(classifierName==null)return false;
		return classifierName.endsWith("Type")
				&& classifierName.replace("Type", "AugmentationPoint").equals(name);
	}
	static public boolean hasAugmentationPointName(Property property){
		String name=NameHelper.getNIEMName(property);
		return name.endsWith("AugmentationPoint");
	}
...
//				&&property.isDerivedUnion()  // still count it as abstract if not an xsd any
	...
	static public boolean isXSDAttribute(Property property){
		if(isXSDAny(property))return false;
		return isXSDAttributeKind(property);
	}
	static public boolean isModelGroupDefinition(Classifier classifier){
		if(classifier==null)return false;
		Element owner=classifier.getOwner();
		if((owner==null)||!(owner instanceof Package))return false;
		return modelGroupDefinitionPackageName.equals(((Package)owner).getName());
	}
	static public boolean isXSDAttributeKind(Property property){
		Type type=property.getType();
		if(isChoice(type))return false;
		
		if(isList(property.getClassifier()))return false;
		Stereotype stereotype=getPsmStereotype(XSDPropertyStereotypeName);
		String name=NameHelper.getNIEMName(property);
...
//		return StereotypesHelper.hasStereotype(element,stereotype);
	...
	static public boolean isNamespaceICISM(Package element){
		return "urn:us:gov:ic:ism".equals(getNamespaceTargetNamespace(element));
	}
	static public boolean isNamespaceICINTK(Package element){
		return "urn:us:gov:ic:ntk".equals(getNamespaceTargetNamespace(element));
	}
...
//				for(NamedElement supplier:getReferences(subsetProperty)){
	...
	static public boolean isLocalVocabulary(Element element){
		if(element==null)return false;
		if(!(element instanceof Enumeration))return false;
		return NIEMHelper.hasStereotype(element,NIEMHelper.getCommonStereotype(LocalVocabularyStereotypeName));
	}
	...
	static public Stereotype getLocalVocabularyStereotype(){
		return getCommonStereotype(LocalVocabularyStereotypeName);
	}
	...
	static public boolean isConformantExtensionSchemaDocument(Package informationModel){
		return
	...
	static public boolean isConformantReferenceSchemaDocument(Package informationModel){
		return
	...
	static public void removeElement(Element element){
     ...
     static public Property getReferenceProperty(Property subsetProperty){
 		...
 		if(referenceProperty==null){
 			...
 			// if subsetProperty is in propertyholder, then scan for a property holder match
 			if(isPropertyHolder(subsetProperty.getClassifier())){
 				Package subsetModel=getNearestInformationModel(subsetProperty);
 				Package informationModel=NIEMHelper.getReferencesReferenceModelInformationModel(subsetModel);
 				for(Type type:informationModel.getOwnedType()){
 					if(isPropertyHolder(type) && (type instanceof Classifier)){
 						Classifier referenceClassifier=(Classifier)type;
 						 ...
 						 if(referenceProperty!=null)break;
 					}
 				}
 			}
 			if(referenceProperty==null){
