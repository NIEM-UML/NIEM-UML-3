 ...
 *
 */
public class NDR9_23_AnAugmentationReferenceMustBeLastParticle extends NameValidationRule 
	//implements Expression
{
	public NDR9_23_AnAugmentationReferenceMustBeLastParticle(){}
    /**
     * {@inheritDoc}
     */
    @Override
    ...
    protected boolean isNameInvalid(NamedElement namedElement){
    	...
    	if(!NIEMHelper.isAugmentationPoint(subsetProperty))return false;
		Classifier classifier=subsetProperty.getClassifier();
		if(NIEMHelper.isPropertyHolder(classifier))return false;
		List<Property> properties=NIEMHelper.getOwnedAttribute(classifier);
		return properties.get(properties.size()-1)!=subsetProperty;
	 	}
/* no fix
    @Override
