...
public class AssociationOperations
	...
	static public Property getMemberEnd(Association association,String name, Type type, boolean ignoreCase,
			EClass eClass) {
		memberEndLoop : for (Property memberEnd : association.getMemberEnd()) {
	...
	static public Property getMemberEnd(Association association,String name, Type type) {
		return getMemberEnd(association,name, type, false, null);
	}

	static public Property createOwnedEnd(Association association,String name, Type type) {
		return createOwnedEnd(association,name, type, UMLPackage.Literals.PROPERTY);
	}
	
	static public Property createOwnedEnd(Association association,String name, Type type, EClass eClass) {
		Property newOwnedEnd = (Property) create(eClass);
		association.getOwnedEnd().add(newOwnedEnd);
		if (name != null)
	...
	public static boolean isBinary(Association association) {
		return association.getMemberEnd().size() == 2;
	}
	...
	public static EList<Type> getEndTypes(Association association) {
		...
		for (Property memberEnd : association.getMemberEnd()) {
			Type endType = (Type) memberEnd.eGet(
					com.nomagic.uml2.ext.magicdraw.metadata.UMLPackage.Literals.TYPED_ELEMENT__TYPE, false);
		...
		return new UnionEObjectEList<Type>((InternalEObject) association,
				com.nomagic.uml2.ext.magicdraw.metadata.UMLPackage.Literals.ASSOCIATION__END_TYPE, endTypes.size(),
			endTypes.toArray());
	}
