...
public class MessageOperations
	...
	public static boolean validateSignatureIsOperation(Message message,
		...
		if (signature instanceof Operation) {
			List<ValueSpecification> arguments = message.getArgument();

			if (!arguments.isEmpty()) {
				EList<Parameter> parameters = new UniqueEList.FastCompare<Parameter>(
					((Operation) signature).getOwnedParameter());

				if (message.getMessageSort() == MessageSortEnum.REPLY) {

					for (Iterator<Parameter> p = parameters.iterator(); p
						.hasNext();) {

						if (p.next().getDirection() == ParameterDirectionKindEnum.IN) {
							p.remove();
						}
					}
				}
				...
				if (arguments.size() != parameters.size()) {
					...
					while (a.hasNext() && p.hasNext()) {
						...
						if (argumentType == null
							? parameterType != null
							: !TypeOperations.conformsTo(argumentType,parameterType)) {
	...
	public static boolean validateSignatureIsSignal(Message message,
		...
		if (signature instanceof Signal) {
			...
			if (!arguments.isEmpty()) {
				Collection<Property> attributes = ((Signal) signature)
					.getAttribute();
				...
				if (arguments.size() != attributes.size()) {
					...
					while (ma.hasNext() && sa.hasNext()) {
						...
						if (argumentType == null
							? attributeType != null
							: !TypeOperations.conformsTo(argumentType,attributeType)) {
		...
		if (!result && diagnostics != null) {
			diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING,
				UMLValidator.DIAGNOSTIC_SOURCE,
				UMLValidator.MESSAGE__SIGNATURE_IS_SIGNAL, NIEMResourceLoader.INSTANCE
	...
	public static MessageKind getMessageKind(Message message) {
		...
		return message.eGet(UMLPackage.Literals.MESSAGE__SEND_EVENT, false) == null
			? (message.eGet(UMLPackage.Literals.MESSAGE__RECEIVE_EVENT, false) == null
				? MessageKindEnum.UNKNOWN
				: MessageKindEnum.FOUND)
			: (message.eGet(UMLPackage.Literals.MESSAGE__RECEIVE_EVENT, false) == null
				? MessageKindEnum.LOST
				: MessageKindEnum.COMPLETE);
		*/		
	}
