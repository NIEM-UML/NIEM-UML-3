...
public class XMLInstanceGenerator {
	...
	public XMLInstanceGenerator() {
		//super();
	}

	public XMLInstanceGenerator(String grammarURI, CMDocument cmDocument) {
			...
			*/
			{
				...
				log("XMLInstanceGenerator XMLSchemaValidationChecker "+cmDocument);
				XMLSchemaValidationChecker validator = new XMLSchemaValidationChecker();
				log("XMLInstanceGenerator XMLSchemaValidationChecker "+validator);
				if (!validator.isValid(uri)) {
					title = "ERROR: INVALID GRAMMER";//XMLWizardsMessages._UI_INVALID_GRAMMAR_ERROR;
					message = "ERROR: Schema invalid";// XMLWizardsMessages._UI_LABEL_ERROR_SCHEMA_INVALID_INFO;
				}
				else if (cmDocument != null) {
					int globalElementCount = cmDocument.getElements().getLength();
					if (globalElementCount == 0) {
						title = "No Root Elements";//XMLWizardsMessages._UI_WARNING_TITLE_NO_ROOT_ELEMENTS;
						message = "WARNING: No Root Elements";//XMLWizardsMessages._UI_WARNING_MSG_NO_ROOT_ELEMENTS;
					}
				}
			}
		}
			...
			title = "No Root Elements";//XMLWizardsMessages._UI_WARNING_TITLE_NO_ROOT_ELEMENTS;
			message = "WARNING: URI NOT FOUND: "+uri;//XMLWizardsMessages._UI_WARNING_URI_NOT_FOUND_COLON + " " + uri; //$NON-NLS-1$
		}
		errorInfo[0] = title;
		errorInfo[1] = message;
		log("XMLInstanceGenerator done "+cmDocument+", "+title+", "+message);
...
*/
	...
	public void createEmptyXMLDocument(IFile newFile) throws Exception {
		String charSet = null;//getUserPreferredCharset();
...
*/
	public void createXMLDocument(String xmlFileName) throws Exception {
		IContentType contentType = null;//Platform.getContentTypeManager().findContentTypeFor(xmlFileName);
		String charset = null;
		if (contentType != null) {
			charset = contentType.getDefaultCharset();
		}
//		ByteArrayOutputStream outputStream = createXMLDocument(xmlFileName, charset);
		OutputStream outputStream = createXMLDocument(xmlFileName, charset);
		/*
	...
	public void createXMLDocument(File newFile/*, String xmlFileName*/) throws Exception {
//		public void createXMLDocument(IFile newFile, String xmlFileName) throws Exception {
		String charset = null;//newFile.getCharset();
		String xmlFileName=newFile.getAbsolutePath();
		createXMLDocument(xmlFileName, charset);
		/*
		ByteArrayOutputStream outputStream = createXMLDocument(xmlFileName, charset);
...
//	public ByteArrayOutputStream createXMLDocument(String xmlFileName, String charset) throws Exception {
	public OutputStream createXMLDocument(String xmlFileName, String charset) throws Exception {
		if (charset == null) {
...
//log("XMLInstanceGenerator elementDeclaration "+cmElementDeclaration+", "+getRootElementName()+", "+nameNodeMap+", "+cmDocument);
