 ...
 *
 */
public class NDR12_23_AssociationType extends NameValidationRule
{
    /**
     * {@inheritDoc}
     */
...
//        addGeneralizationConfiguration(configMap);
     ...
     protected boolean isNameInvalid(NamedElement namedElement){
		...
		Classifier general=NIEMHelper.getBaseClassifier((Classifier)namedElement);
		 if((general!=null)&&!NIEMHelper.isAssociationType(general))return true;
		/*
    	 for(Classifier general:NIEMHelper.getBaseClassifier(object)getGeneral((Classifier)namedElement)){
    		 if(!NIEMHelper.isAssociationType(general))return true;
    	 }
    	 */
    	 return false;
     }
