 ...
 *
 */
public class NDR12_27_MetadataApplies extends NameValidationRule
{
    /**
     * {@inheritDoc}
     */
    ...
    protected boolean isNameInvalid(NamedElement namedElement){
	   	 // Usage supplier must be object, association, or external adapter types
    	// (not role, metadata,augmentation)
    	...
    	for(NamedElement supplier:((Usage)namedElement).getSupplier()){
    		...
    		if(
    				//((supplierClass instanceof Class)&&NIEMHelper.isRoleType((Class)supplierClass))
    				NIEMHelper.isMetadataType(supplierClass)
    				||NIEMHelper.isAugmentationType(supplierClass)
    				||NIEMHelper.isPropertyHolder(supplierClass)
    				||NIEMHelper.isChoice(supplierClass)
    				||!NIEMHelper.isComplexType(supplierClass)
    				)return true;
    	}
	   	 return false;
    }
