...
public class OperationOperations
	...
	public static boolean isOrdered(Operation operation) {
		Parameter returnResult = OperationOperations.getReturnResult(operation);
	...
	public static EList<Parameter> returnResult(Operation operation) {
		...
		for (Parameter ownedParameter : operation.getOwnedParameter()) {

			if (ownedParameter.getDirection() == ParameterDirectionKindEnum.RETURN) {
	...
	public static void setIsOrdered(Operation operation, boolean newIsOrdered) {
		...
		if (returnResult == null) {
			returnResult = BehavioralFeatureOperations.createOwnedParameter(operation,null, null);
			returnResult.setDirection(ParameterDirectionKindEnum.RETURN);
		}

		returnResult.setOrdered(newIsOrdered);
	}
	...
	public static void setIsUnique(Operation operation, boolean newIsUnique) {
		...
		returnResult.setUnique(newIsUnique);
	}
...
//		returnResult.setLower(newLower);
		MultiplicityElementOperations.setLower(returnResult,newLower);
	}
...
//		returnResult.setUpper(newUpper);
		MultiplicityElementOperations.setUpper(returnResult,newUpper);
	}
...
//			EList<Parameter> returnResult = operation.returnResult();
			EList<Parameter> returnResult = returnResult(operation);
			int returnResultSize = returnResult.size();
//			EList<Parameter> opReturnResult = op.returnResult();
			EList<Parameter> opReturnResult = returnResult(op);
			...
			if (ownedParametersSize == opOwnedParameters.size()
				...
				for (int i = 0; i < ownedParametersSize; i++) {
					...
					if (opOwnedParameterType == null
						...
						: !TypeOperations.conformsTo(opOwnedParameterType,ownedParameterType)) {
				...
				for (int i = 0; i < returnResultSize; i++) {
					...
					if (opReturnResultType == null
						...
						: !TypeOperations.conformsTo(opReturnResultType,returnResultType)) {
