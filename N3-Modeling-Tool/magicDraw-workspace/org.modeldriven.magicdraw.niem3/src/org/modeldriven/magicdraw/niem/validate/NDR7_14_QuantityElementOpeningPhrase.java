 ...
 *
 */
public class NDR7_14_QuantityElementOpeningPhrase extends OpeningPhraseRule
{
	protected String getOpeningPhraseMatch() {
//		return "A .*count/number of .*";
		return "(?s)A .*count of .*";
	}
    ...
    protected boolean isOpeningPhraseValid(String body){
    	...
    	|| body.matches("(?s)A .*number of .*");
    }
	
	@Override
	protected boolean isComponentClass(Element namedElement) {
		return NIEMHelper.isXSDElement((Property)namedElement)
			//&&isQuantity(namedElement.getName());
			&&isQuantity(NameHelper.getNIEMName((Property)namedElement));
