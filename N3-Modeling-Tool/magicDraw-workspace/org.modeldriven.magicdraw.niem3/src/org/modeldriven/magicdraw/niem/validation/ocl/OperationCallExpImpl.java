...
public class OperationCallExpImpl
	...
	public void setReferredOperation(Operation newReferredOperation) {
		...
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET,
				...
				oldReferredOperation, referredOperation));
		/*
		log("OperationCallExp.setReferredOperation "+referredOperation.getQualifiedName()+", "+getName()
				
				+", source="+(source==null?"null":
		(source.getName()+", "+source+"("+source.getClass().getName()+"), source type="+
				(source.getType()==null?"null":(source.getType().getQualifiedName()))
			))
				);
				*/
	}
	...
	public int getOperationCode() {
		...
		/*
		log("OperationCall.getOperationCode "+operationCode
				+", operation "+getName()
				+", referredOperation="+getReferredOperation()+", "+(getReferredOperation()==null?"null":getReferredOperation().getName())
				);
*/
	...
	public void setOperationCode(int newOperationCode) {
		...
		// correction for isStandardLibraryFeature problem
		if((operationCode==0)&&(getReferredOperation()!=null)){
			String resolvedName=getReferredOperation().getName();
			operationCode = OCLStandardLibraryUtil.getOperationCode(resolvedName);
			if(operationCode!=0){
				OCLExpression<Classifier> source=getSource();
				log("OperationCall.setOperationCode OVERRIDE to "+operationCode+" for "+resolvedName
						+", source "+(source==null?"null":source.getName()+"("+source.getClass().getName()+"), source type="
								+(source.getType()==null?"null":source.getType().getQualifiedName()+"("+source.getType().getClass().getName()+")")
								)
						);
			}
		}
		/*
		if((getReferredOperation()!=null) && "oclAsType".equals(getReferredOperation().getName()))
			log("OperationCall.setOperationCode "+getReferredOperation().getName()+" to "+operationCode);
		if((operationCode==0)&&getReferredOperation()!=null){
			String resolvedName=getReferredOperation().getName();
			operationCode = OCLStandardLibraryUtil.getOperationCode(resolvedName);
			if(operationCode!=0){
				log("OperationCall.setOperationCode OVERRIDE to "+operationCode);
			}
		}
		*/
	}
...
//		return (Classifier) super.getType();
		...
		//if(result!=null)log("OperationCall.getType "+getName()+", "+(result==null?"null":(result.getQualifiedName()+", "+result.getClass().getName())));
		//log("OperationCall.getType "+getName()+", "+(result==null?"null":(result.getQualifiedName()+", "+result.getClass().getName())));
	...
	public void setType(Classifier typeIn) {
		...
		//if(operationCode==PredefinedType.OCL_AS_TYPE){
		//if(getReferredOperation()!=null)System.out.println("setType "+getReferredOperation().getName()+", "+((type==null)?"null":type.getQualifiedName()));
		if((type!=null)&&(getReferredOperation()!=null)&&("oclAsType".equals(getReferredOperation().getName()))){
			for(OCLExpression<Classifier> arg:getArgument()){
				if(TypeExp.class.isInstance(arg)&&!type.equals(((TypeExp)arg).getReferredType())){
					type=((TypeExp)arg).getReferredType();
					log("OperationCall.setType OVERRIDE/FIXUP type to "+type+", "+(type==null?"null":type.getQualifiedName())
							+", original type "+typeIn+", "+(typeIn==null?"null":typeIn.getQualifiedName())
							+", operation "+getName()
							+", referredOperation="+getReferredOperation()+", "+(getReferredOperation()==null?"null":getReferredOperation().getName())
							);
				}
		...
		//type=referredType;
		super.setType((Type) type);
		//if(type!=null)
		/*
		log("OperationCall.setType "+getName()+", original type "+(typeIn==null?"null":(typeIn.getQualifiedName()+", "+typeIn.getClass().getName()))
				+", used type "+(type==null?"null":type.getQualifiedName())
				+", source="+(source==null?"null":
		(source.getName()+", "+source+"("+source.getClass().getName()+"), source type="+
				(source.getType()==null?"null":(source.getType().getQualifiedName()))
			))
		...
		*/
		/*
			for(OCLExpression<Classifier> arg:getArgument()){
				log("OperationCall.setType arg "+arg+"("+arg.getClass().getName()+", "+arg.getName());
			}
		*/
	}
