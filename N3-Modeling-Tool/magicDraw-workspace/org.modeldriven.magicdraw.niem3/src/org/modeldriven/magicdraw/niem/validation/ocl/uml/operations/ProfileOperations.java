...
//		for (Class referencedMetaclass : profile.getReferencedMetaclasses()) {
		for (Class referencedMetaclass : getReferencedMetaclasses(profile)) {
			...
			if (result) {
				...
				for (Classifier parent : ClassifierOperations.allParents(referencedMetaclass)) {
...
//		EList<Model> metamodels = new UniqueEList.FastCompare<Model>(profile.getReferencedMetamodels());
		EList<Model> metamodels = new UniqueEList.FastCompare<Model>(getReferencedMetamodels(profile));

//		for (Class referencedMetaclass : profile.getReferencedMetaclasses()) {
		for (Class referencedMetaclass : getReferencedMetaclasses(profile)) {

//			metamodels.add(referencedMetaclass.getModel());
			metamodels.add(ClassOperations.getModel(referencedMetaclass));
		}
...
//		ENamedElement definition = profile.getDefinition(classifier);
		ENamedElement definition = ProfileOperations.getDefinition(profile,classifier);
...
//		Stereotype ownedStereotype = (Stereotype) profile.createOwnedType(name,
		Stereotype ownedStereotype = (Stereotype) ProfileOperations.createOwnedType(profile,name,
				UMLPackage.Literals.STEREOTYPE);
		ownedStereotype.setAbstract(isAbstract);
...
//		return profile.getDefinition() != null;
		return ProfileOperations.getDefinition(profile) != null;
	}
	 ...
	 * @generated NOT
	 */
	public static Package define(Profile profile) {
		return profile;
	}
	/*
...
//		return profile.define(options, null, null);
		return ProfileOperations.define(profile,options, null, null);
	}
*/
	/**
	 * <!-- begin-user-doc -->
	 ...
	 * @generated NOT
	 */
	public static Package define(Profile profile, Map<String, String> options,
			DiagnosticChain diagnostics, Map<Object, Object> context) {
		return profile;
	}
	/*
...
*/
	 ...
	 * @generated NOT
	 */
	public static Profile getDefinition(Profile profile) {
		return profile;
	}
/*
...
//		return getDefinition(profile, namedElement, profile.getDefinition());
		return getDefinition(profile, namedElement, getDefinition(profile));
	}
*/
	/**
	 * <!-- begin-user-doc -->
	 ...
	 * @generated NOT
	 */
	public static EList<Class> getReferencedMetaclasses(
			Profile profile) {
		EList<Class> referencedMetaclasses = new UniqueEList.FastCompare<Class>();
	...
	public static EList<Model> getReferencedMetamodels(Profile profile) {
		...
		for (PackageImport metamodelReference : profile
			...
			Package importedPackage = metamodelReference
				.getImportedPackage();
	...
	protected static EList<Extension> getOwnedExtensions(Profile profile,
		...
		for (Extension extension : EcoreUtil.<Extension> getObjectsByType(
			profile.getPackagedElement(), com.nomagic.uml2.ext.magicdraw.metadata.UMLPackage.Literals.EXTENSION)) {
