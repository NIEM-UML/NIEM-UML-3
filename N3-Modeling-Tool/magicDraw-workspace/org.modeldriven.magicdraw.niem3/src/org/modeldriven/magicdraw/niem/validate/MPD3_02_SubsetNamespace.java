...
/**
 * Validation rule for:
 * Each schema in a schema subset derived from a schema reference set 
 * bears the same (target) namespace as the schema in the reference set on which it is based.
 *
 */
public class MPD3_02_SubsetNamespace extends NameValidationRule
{
    /**
     * {@inheritDoc}
     */
    ...
    protected boolean isNameInvalid(NamedElement namedElement){
    	Package subsetInformationModel=(Package)namedElement;
    	if(!NIEMHelper.isInformationModel(subsetInformationModel))return false;
   	 if(!NIEMHelper.getNamespaceIsConformant(subsetInformationModel))return false;
   	 if(!NIEMHelper.isInformationModelDefaultPurposeSubset(subsetInformationModel))return false;
   	 // must have a <<References>> to a reference model with same namespace
   	 Package referenceModel=NIEMHelper.getReferencesReferenceModelInformationModel(subsetInformationModel);
   	 if(referenceModel==null)return true;
   	 ...
   	 if((referenceNamespace==null)||(subsetNamespace==null))return true;
   	 return !referenceNamespace.equals(subsetNamespace);
	 	}
