 ...
 *
 */
public class NDR7_14_IndicatorElementOpeningPhrase extends OpeningPhraseRule
{
	protected String getOpeningPhraseMatch() {
//		return "True if .*; false otherwise/if .*";
		return "(?s)"+"True if .*; false otherwise.*";
	}
    protected boolean isOpeningPhraseValid(String body){
    	if(body==null)return false;
    	return body.matches(getOpeningPhraseMatch())
    	|| body.matches("(?s)"+"True if .*; false if .*");
    }
...
//			&&isIndicator(namedElement.getName());
			&&isIndicator(NameHelper.getNIEMName((Property)namedElement));
	}
