...
public class ConnectorOperations
	...
	public static boolean validateTypes(Connector connector,
		...
		if (connectorType != null) {
			...
			Iterator<Property> memberEnds = connectorType.getMemberEnd()
				.iterator();
			...
			while (ends.hasNext() && memberEnds.hasNext()) {
				...
				if (type == null
					? memberEndType != null
					: !TypeOperations.conformsTo(type,memberEndType)) {
		...
		if (!result && diagnostics != null) {
			diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING,
				...
				NIEMResourceLoader.INSTANCE.getString("_UI_Connector_Types_diagnostic", //$NON-NLS-1$
		...
		while (ends.hasNext()) {
			...
			if (connector.getOwner() != owner) {
				...
				if (diagnostics != null) {
					diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING,
						UMLValidator.DIAGNOSTIC_SOURCE,
						UMLValidator.CONNECTOR__ROLES, NIEMResourceLoader.INSTANCE
	...
	public static boolean validateBetweenInterfacesPorts(Connector connector,
		...
		if (connector.getKind() == ConnectorKindEnum.DELEGATION
			&& ends.size() == 2) {
		...
		if (connector.getKind() == ConnectorKindEnum.DELEGATION
			...
			if (!provided1.isEmpty() && !(role2 instanceof Port)
				...
				if (toType instanceof BehavioredClassifier
					&& (!BehavioredClassifierOperations.getAllImplementedInterfaces(((BehavioredClassifier) toType)).containsAll(provided1))) {
	...
	public static boolean validateAssemblyConnector(Connector connector,
		...
		if (connector.getKind() == ConnectorKindEnum.ASSEMBLY
			&& ends.size() == 2) {
