...
import org.modeldriven.magicdraw.niem.validation.ocl.uml.operations.RegionOperations;
...
//        Package result = pkg.getNestedPackage(name);
        Package result = PackageOperations.getNestedPackage(pkg,name);
...
//            result = pkg.getNestedPackage(AbstractOCLAnalyzer.unescape(name));
            result = PackageOperations.getNestedPackage(pkg,AbstractOCLAnalyzer.unescape(name));
        }
...
//        NamedElement result = ns.getMember(name, false, type);
        NamedElement result = NamespaceOperations.getMember(ns,name, false, type);
        
        if ((result == null) && AbstractOCLAnalyzer.isEscaped(name)) {
            // try the unescaped name
//            result = ns.getMember(AbstractOCLAnalyzer.unescape(name), false, type);
            result = NamespaceOperations.getMember(ns,AbstractOCLAnalyzer.unescape(name), false, type);
        }
...
//        Vertex result = region.getSubvertex(name);
        Vertex result = RegionOperations.getSubvertex(region,name);

        if ((result == null) && AbstractOCLAnalyzer.isEscaped(name)) {
            // try the unescaped name
//            result = region.getSubvertex(AbstractOCLAnalyzer.unescape(name));
            result = RegionOperations.getSubvertex(region,AbstractOCLAnalyzer.unescape(name));
        }
...
//        result.addAll(classifier.getAssociations());
        result.addAll(TypeOperations.getAssociations(classifier));
        for (Classifier parent : ClassifierOperations.allParents(classifier)) {
            result.addAll(TypeOperations.getAssociations(parent));
        }
