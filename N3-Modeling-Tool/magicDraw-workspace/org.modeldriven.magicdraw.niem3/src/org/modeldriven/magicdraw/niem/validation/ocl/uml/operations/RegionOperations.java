...
public class RegionOperations
	...
	public static  Vertex getSubvertex(Region region,java.lang.String name){
		Vertex result=null;
		for(Vertex vertex:region.getSubvertex())if(name.equals(vertex.getName()))return vertex;
		return result;
	}
	...
	public static Classifier redefinitionContext(Region region) {
		StateMachine sm = containingStateMachine(region);
//		StateMachine sm = region.containingStateMachine();
	...
	public static StateMachine containingStateMachine(Region region) {
		...
		if (stateMachine == null) {
			...
			return state == null
				? null
						: StateOperations.containingStateMachine(state);
//				: state.containingStateMachine();
		} else {
			return stateMachine;
		}
...
//				return container != null && container.belongsToPSM();
				return container != null && RegionOperations.belongsToPSM(container);
			}
	...
	public static boolean isConsistentWith(Region region,
		...
		if (redefinee != null && RedefinableElementOperations.isRedefinitionContextValid(redefinee,region)) {
//		if (redefinee != null && redefinee.isRedefinitionContextValid(region)) {
			...
			for (Vertex redefineeSubvertex : redefineeRegion.getSubvertex()) {
	...
	protected static EList<Vertex> getAllSubvertices(Region region) {
		...
		for (Region extendedRegion : getAllExtendedRegions(region)) {
			allSubvertices.addAll(extendedRegion.getSubvertex());
		}
	...
	protected static EList<Transition> getAllTransitions(Region region) {
		...
		for (Region extendedRegion : getAllExtendedRegions(region)) {
			...
			allTransitions.addAll(extendedRegion.getTransition());
		}
