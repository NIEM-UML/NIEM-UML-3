...
public class OCLFactoryImpl implements OCLFactory {
    ...
    public <C, O, P> org.eclipse.ocl.types.TupleType<O, P> createTupleType(
        ...
        if (!parts.isEmpty()) {
            ...
            //System.out.println("createTupleType properties "+properties);
            for (TypedElement<C> part : parts) {
            	String name=part.getName();
            	C type=part.getType();
            	Property property=(Property) uml.createProperty(name, type);
              //  System.out.println("SUPPRESS createTupleType part "+name+", "+type+", "+property);
if(true)continue;                
