 ...
 *
 */
public class NDR7_14_AugmentationElementOpeningPhrase extends OpeningPhraseRule
{
	@Override
	...
	protected boolean isComponentClass(Element namedElement) {
		if(Generalization.class.isInstance(namedElement)){
			...
			boolean result= NIEMHelper.isAugmentationType(generalization.getGeneral())
					&& !NIEMHelper.isAugmentationType(generalization.getSpecific());
			return result;
		}
		return NIEMHelper.isXSDElement((Property)namedElement)
			&&(((Property)namedElement).getType()!=null)
			&&NIEMHelper.isAugmentationType((Classifier)((Property)namedElement).getType());
	}
