...
public class NamedElementOperations
	...
	static public boolean isSetVisibility(NamedElement namedElement) {
		//return (eFlags & VISIBILITY_ESETFLAG) != 0;
		return namedElement.getVisibility()!=null;
	}
	...
	protected static Dependency createDependency(NamedElement namedElement,
		...
		Package package_ = ElementOperations.getNearestPackage(namedElement);
...
//		Dependency dependency = (Dependency) package_.createPackagedElement(
		Dependency dependency = (Dependency) PackageOperations.createPackagedElement(package_,
			null, eClass);

		dependency.getClient().add(namedElement);
...
//		return getQualifiedName(namedElement, namedElement.separator());
		return getQualifiedName(namedElement, NamedElementOperations.separator(namedElement));
	}
...
//			EList<String> namesOfN = ns.getNamesOfMember(n);
			EList<String> namesOfN = NamespaceOperations.getNamesOfMember(ns,n);

//			for (String nameOfNamedElement : ns.getNamesOfMember(namedElement)) {
			for (String nameOfNamedElement : NamespaceOperations.getNamesOfMember(ns,namedElement)) {

				if (namesOfN.contains(nameOfNamedElement)) {
					QvtCommonAction.log("WARNING: indistinguishable element "+namedElement.getQualifiedName());
					return false;
				}
			}
		}
	 ...
	 * @generated NOT
	 */
	public static EList<Package> allOwningPackages(
			NamedElement namedElement) {
		EList<Package> allOwningPackages = new UniqueEList.FastCompare<Package>();

		while ((namedElement = (NamedElement) getOwningElement(namedElement,
				UMLPackage.Literals.PACKAGE, true)) != null) {

			allOwningPackages.add((Package) namedElement);
		}
