 ...
 *
 */
public class NDR8_37_ElementTypeIsNotASimpleType extends NameValidationRule 
	//implements Expression
{
	public NDR8_37_ElementTypeIsNotASimpleType(){}
    /**
     * {@inheritDoc}
     */
    @Override
    ...
    static protected String getNameInvalidMessage(NamedElement namedElement){
    	...
    	// check for elements
    	if(NIEMHelper.isXSDAttribute(subsetProperty))return null;
    	Type type=subsetProperty.getType();
    	if((type==null)||!(type instanceof DataType))return null;
    	if(NIEMHelper.isDataTypeSimpleType((DataType)type)){
    		// could be proxy
    		if(!NIEMHelper.isXMLPrimitiveTypesLibrary(NIEMHelper.getNearestPackage(type))){
    		return "type "+type.getQualifiedName()+" is a simple type"; 
    		}
    	}
    	return null;
	 	}
