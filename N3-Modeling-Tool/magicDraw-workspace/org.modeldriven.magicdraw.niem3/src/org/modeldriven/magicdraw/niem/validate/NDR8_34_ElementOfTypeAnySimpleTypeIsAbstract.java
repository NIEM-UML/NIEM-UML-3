 ...
 *
 */
public class NDR8_34_ElementOfTypeAnySimpleTypeIsAbstract extends NameValidationRule 
	//implements Expression
{
	public NDR8_34_ElementOfTypeAnySimpleTypeIsAbstract(){}
    /**
     * {@inheritDoc}
     */
    @Override
    ...
    static protected String getNameInvalidMessage(NamedElement namedElement){
    	...
    	if(isAnySimpleType(type)){
    		if(!subsetProperty.isDerivedUnion()){
    			return "Property must be abstract by setting isDerivedUnion";
    		}
    	}
    	return null;
	 	}
   ...
   static  protected boolean isAnySimpleType(NamedElement ne){
    	if(!NIEMHelper.isXMLPrimitiveTypesLibrary(NIEMHelper.getNearestPackage(ne)))return false;
    	return("anySimpleType".equals(ne.getName()));
    }
